Shader "Hidden/CRTFilter"
{
    ...
    SubShader
    {
        ...
        Pass
        {
			...
			uniform fixed m_scanlinesRange;
			uniform fixed m_scanlinesColorGlitchA;
			uniform fixed m_scanlinesColorGlitchB;
			...
			{
				half odd = fmod(floor(uv.y * m_resY * 100), 4);
			...
			{
				fixed c = lerp(1, min(0.8, sin(frac(uv * m_resY / 4.0) * pi)), m_scanlinesStrength);
				fixed rDiff = (1 - col.r * m_scanlinesRange);
				fixed gDiff = (1 - col.g * m_scanlinesRange);
				fixed bDiff = (1 - col.b * m_scanlinesRange);
				fixed4 newCol = fixed4(col.r * (1 - ((1 - c) * rDiff)), col.g * (1 - ((1 - c) * gDiff)), col.b * (1 - ((1 - c) * bDiff)), col.a);
				return newCol;
			}

			fixed4 colorEnhancement(fixed4 fragColor, in fixed2 fragCoord )
			{    
				fixed2 uv = fragCoord; //fragCoord.xy / iResolution.xx;
    
				fixed2 uv2 = frac(uv * m_resX / 4.0); // add and subtract half a unit to get centered lines ;
				...
				// red
				if (abs(uv2.x) >= d1 && abs(uv2.x) < d2)
				{
					fixed rDiff = (1 - fragColor.r);
					fragColor = fixed4(fragColor.x * m_scanlinesColorGlitchA, fragColor.y / m_scanlinesColorGlitchB, fragColor.z / m_scanlinesColorGlitchB, 1.0);
				}
				//green
				if (abs(uv2.x) >= d2 && abs(uv2.x) < d3)
				{
					fragColor = fixed4(fragColor.x / m_scanlinesColorGlitchB, fragColor.y * m_scanlinesColorGlitchA, fragColor.z / m_scanlinesColorGlitchB, 1.0);
				}
				// blue
				if (abs(uv2.x) >= d3)
				{
					fragColor = fixed4(fragColor.x / m_scanlinesColorGlitchB, fragColor.y / m_scanlinesColorGlitchB, fragColor.z * m_scanlinesColorGlitchA, 1.0);
				}
